...
class MLP_Trainer:
    def __init__(self, config_dir, model, train_loader, val_loader, test_loader, optimizer, criterion,
                 ...
                 pinn_reg_factor=1, num_epochs=500, eta=1e-3, model_loss_tolerance=1e-4, taylor_offset=1e-3, save_checkpoint_iter=50, 
                 gaussian_mean=0.0, gaussian_var=0.0, gaussian_scale=0.0, checkpoint_path=None, device=None, is_linear=False):
        ...
        self.mse_mape_save_path = f"{self.config_dir}/mlp_metrics.txt"
    ...
    def train_model(self):
        ...
        for x_batch, y_batch, y_data_batch in self.train_loader:
            ...
            # x_input = torch.cat([x_batch, y_hat[:, :self.num_ys]], dim=1)
            # # x_input = torch.cat([x_input, y_hat_delta[:, :self.num_ys]], dim=1)
            ...
            # print("MLP Train y_hat shape: ", y_hat.shape)
            # print("MLP Train x_input shape: ", x_input.shape)
    ...
    def test_model(self):
        ...
        for x_test_batch, y_test_batch, y_data_test_batch in self.test_loader:
            ...
            # print("MLP Test y_hat shape: ", y_hat.shape)
            # print("MLP Test x_input shape: ", x_input.shape)
            eq_res = self.orig_eq_viol_fn(y_hat, x_input)
            ineq_res = self.orig_ineq_viol_fn(y_hat, x_input)
